// This will be done in a component-based architecture

const SCENE_SIZE = 1000;

let createOrbital = (scene) => {
    let orbitSphere = BABYLON.MeshBuilder.CreateSphere(
        "orbitSphere",
    ...
    orbit.updateParticle = (p) => {
        ...
        p.scale = new BABYLON.Vector3(sd, sd, sd).multiplyByFloats(
            0.15,
            0.15,
            0.7 + d * 0.3
        );
...
let createJunkCloud = (scene) => {
    ...
    junkMatA.diffuseFresnelParameters.leftColor = new BABYLON.Color3(
        0.28,
        0.63,
        0.96
    );
    junkMatA.diffuseFresnelParameters.rightColor = new BABYLON.Color3(
        0,
        0.1,
        0.19
    );
    ...
    junkMatA.emissiveFresnelParameters.leftColor = new BABYLON.Color3(
        0.25,
        0.48,
        0.69
    );
    junkMatA.emissiveFresnelParameters.rightColor = new BABYLON.Color3(
        0,
        0.1,
        0.19
    );
    ...
    junkMatB.diffuseFresnelParameters.leftColor = new BABYLON.Color3(
        0.88,
        0.98,
        0.98
    );
    junkMatB.diffuseFresnelParameters.rightColor = new BABYLON.Color3(
        0.51,
        0.51,
        0.51
    );
    ...
    junkMatB.emissiveFresnelParameters.leftColor = new BABYLON.Color3(
        0.48,
        0.49,
        0.5
    );
    junkMatB.emissiveFresnelParameters.rightColor = new BABYLON.Color3(
        0.1,
        0.15,
        0.19
    );
...
let createPlanet = (scene) => {
    let water = BABYLON.MeshBuilder.CreateSphere(
        "sphere",
    ...
    let waterShader = new BABYLON.ShaderMaterial(
        "waterShader",
        scene,
        {
            vertex: "retroWater",
    ...
    let ground = BABYLON.MeshBuilder.CreateIcoSphere(
        "ground",
        { size: 1, subdivisions: 32 },
        scene
    );
    ...
    for (let i = 0; i < p.length; i += 3) {
        let n =
            0.9 +
            Math.abs(noise.noise(p[i] * 1.75, p[i + 1] * 1.75, p[i + 2] * 1.75)) *
            0.42;
        let h = new BABYLON.Vector3(
            vData.positions[i],
            vData.positions[i + 1],
            vData.positions[i + 2]
        )
    ...
    ground.material.diffuseFresnelParameters.rightColor = new BABYLON.Color3(
        0.28,
        0.6,
        0.93
    );
    ...
    let atmos = BABYLON.MeshBuilder.CreateSphere(
        "atmos",
        { diameter: 2.25 },
        scene
    );
    ...
    BABYLON.Effect.ShadersStore["atmosOpacityPixelShader"] = `
        ...
        float flow(in vec3 p, in float t)
        {
            ...
            `;
    let opacity = new BABYLON.CustomProceduralTexture(
        "atmosOpacity",
        "atmosOpacity",
        256,
        scene,
...
let createSky = (scene) => {
    var sky = BABYLON.MeshBuilder.CreateSphere(
        "sphere",
        { diameter: SCENE_SIZE, segments: 32 },
        scene
    );
    ...
    let shader = new BABYLON.ShaderMaterial(
        "sky",
        scene,
        {
            vertex: "retroSky",
    ...
    let starCube = BABYLON.MeshBuilder.CreateIcoSphere(
        "starCube",
        { radius: 2 },
        scene
    );
    ...
    stars.resetParicle = (particle) => {
        ...
        dir = dir.normalize();
        particle.position = dir.scale(SCENE_SIZE * 0.5 - 1);
        particle.rotation.x = Math.random() * Math.PI * 2;
        particle.rotation.y = Math.random() * Math.PI * 2;
        particle.rotation.z = Math.random() * Math.PI * 2;
        ...
        let c3 = BABYLON.Vector3.Lerp(
            BABYLON.Vector3.Lerp(c0, c1, Math.random()),
            c2,
            Math.random() * 0.5
        );
...
let createVisualChain = (scene) => {
    ...
    let downsample = new BABYLON.PassPostProcess(
        "downsample",
        0.0,
        scene.activeCamera,
    ...
    let blurPass1 = new BABYLON.BlurPostProcess(
        "glareBlurPass1",
    ...
    let blurPass2 = new BABYLON.BlurPostProcess(
        "glareBlurPass2",
    ...
    let toneMap = new BABYLON.TonemapPostProcess(
        "tonemap",
...
var setupScene = function (scene, canvas) {
    ...
    let d = () => {
        return (
            1.0 -
            (camera.position.length() - camera.lowerRadiusLimit) /
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let engine = new ADEngine(canvas);
    let scene = new ADScene(engine);
